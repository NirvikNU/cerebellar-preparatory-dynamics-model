function fixed = load_hennequin_isn(params, projectRoot)
    ...
    expectedSize = [params.model.numCorticalUnits, ...
        params.model.numCorticalUnits];
    if ~isequal(size(recurrentMatrix), expectedSize)
        error('IsnModel:MatrixSize', 'Official W_rec must be 200-by-200.');
    end
    ...
    if spectralAbscissa >= params.model.isnSpectralAbscissaLimit
        error('IsnModel:UnstableBackbone', ...
...
function hash = file_sha256(path)
    file = fopen(path, 'rb');
    if file < 0
        error('IsnModel:MatrixFile', 'Cannot open official W_rec: %s', path);
    end
