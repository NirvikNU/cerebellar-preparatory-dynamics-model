function task = build_v3_task(params, trialsPerTarget, goTimeMs, targetIndex)
    ...
    if numel(goTimeMs) ~= nTrials
        error('V3Model:GoTimes', 'A go time is required for every trial.');
    end
    if any(~ismember(double(goTimeMs), params.task.delayGridMs))
        error('V3Model:DelayGrid', ...
            'Every go time must belong to the configured V3 delay grid.');
    end
    counts = accumarray(targetIndex(:), 1, [nTargets 1]);
    if any(counts ~= trialsPerTarget)
        error('V3Model:UnbalancedBatch', ...
