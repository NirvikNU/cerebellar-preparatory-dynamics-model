...
## Deterministic stationarity refinement outcome

- The original pre-go velocity loss was `mean((vx^2+vy^2)/0.25^2)` over all trial-time samples strictly before each trial's sampled go time, with weight 100. Masks were verified at 500, 550, and 600 ms: the last included samples were 495, 545, and 595 ms, respectively, and the go sample was excluded. No mask, indexing, `dt`, or plotting-alignment bug was found.
- The update-1,000 model showed genuine late leakage: across the 500:10:600-ms delay/target grid, full-preparation RMS speed was 0.018010 m/s, final-150-ms RMS was 0.034317 m/s, final-100-ms RMS was 0.040857 m/s, and maximum pre-go speed was 0.105437 m/s. Full-interval averaging diluted the larger late-preparatory error.
- A purely behavioral term was added: `mean((vx^2+vy^2)/0.25^2)` over samples satisfying `go-150 ms <= time < go` for each trial. It has no neural-state, trajectory, subspace, PCA, or lesion target. Candidate late-term weights 50 and 100 were each tested for 100 deterministic updates from the identical update-1,000 model, Adam moments, RNG state, and validation set. Weight 50 was selected as the smallest tested strengthening that clearly reduced leakage without degrading reaching.
- Exact-state continuation restored the update-1,000 parameter vector, Adam first/second moments, learning rate, task/noise RNG states, and fixed validation trials. The selected refinement completed the authorized 500 additional deterministic updates in 29.498 minutes. The best validation checkpoint was the final update, 1,500, with validation loss 1.114119649 and training loss 1.152739406; validation was still improving at the imposed stop.
- Canonical deterministic results: mean endpoint error 0.334 mm, worst target-averaged endpoint error 0.540 mm, terminal speed 0.003550 m/s, pre-go RMS speed 0.007502 m/s, final-150-ms RMS 0.012305 m/s, final-100-ms RMS 0.014434 m/s, maximum pre-go speed 0.040286 m/s, mean hold error 0.363 mm, and mean hold speed 0.004138 m/s.
- Across delays 500–600 ms, endpoint error was 0.292–0.353 mm, worst target-averaged error 0.476–0.819 mm, terminal speed 0.003126–0.010758 m/s, full pre-go RMS speed 0.004885–0.016634 m/s, final-150-ms RMS 0.006464–0.032237 m/s, final-100-ms RMS 0.006076–0.039252 m/s, hold error 0.362–0.462 mm, and hold speed 0.003929–0.006316 m/s.
- All reach, terminal-speed, hold, all-target, finite-state, and delay-range criteria pass. The 0.002-m/s pre-go RMS criterion still fails, and late acceleration remains systematic. No additional optimization was launched.
- Analysis-only top-10-PC diagnostics use the final 300 ms before go and first 300 ms after go. Preparatory activity captured by movement PCs was 78.496% of total preparatory variance; movement activity captured by preparatory PCs was 72.901%. Elsayed-style alignment indices were 0.7921 (preparatory variance captured by movement PCs, normalized by optimal preparatory PCs) and 0.7465 in the reverse direction. These subspaces therefore remain strongly overlapping.
- The two-dimensional `Wout` row space captured only 0.0727% of centered preparatory variance, with normalized potent alignment 0.000894. This output-potent result is descriptive only and was not used in training.
- Stage B, neural noise, cerebellar removal, lesion manipulation, and adaptation remain unrun.
