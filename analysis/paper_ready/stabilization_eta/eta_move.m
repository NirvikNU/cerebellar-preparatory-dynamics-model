function out = eta_move(m,go)
    % Frozen paper_move integration/QC, with saved neural states in the same replay.
    x=go; count=size(go,2); out.go=go; out.rateMax=0; out.stateMax=0;
    out.torque=zeros(m.nSamples,2,count); out.states=zeros(m.nSamples,200,count);
    sample=0; stride=round(m.samplingDt/m.dt); out.audit=[];
    for step=0:m.nInternalSteps-1
        ...
        if mod(step,stride)==0
            ...
            out.states(sample,:,:)=permute(x,[3 1 2]);
        end
    ...
    for q=1:8
        ix=(q-1)*30+(1:30); centroids(:,q)=mean(endpoint(:,ix),2);
