function report = paper95_report(root)
    ...
    selectedValues=[point.prIntact point.prBlock point.observed point.expected point.deltaPR point.deficit];
    report.selectedBootstrapMaxError=0;
    for metric=1:6
        v=selectedValues(:,metric); boot=median(v(controls.indices),2);
        directSE=sqrt(sum((boot-sum(boot)/numel(boot)).^2)/(numel(boot)-1));
        report.selectedBootstrapMaxError=max(report.selectedBootstrapMaxError,abs(directSE-report.selected.se(metric)));
    end
    assert(report.selectedBootstrapMaxError<1e-12);
    ...
    report.selectedControlK=point.kControl.';
    report.oldSelectedIndex=grid.oldSelectedIndex; report.gridAudit=jsondecode(fileread(fullfile(dest,'grid_audit.json')));
    report.captureControl=[point.beforeControl point.captureControl point.captureBlockControlK];
    writetable(point(:,{'network','alpha','betaNormalized','kControl','beforeControl','captureControl','captureBlockControlK','denominator'}),fullfile(dest,'selected_K_audit.csv'));
    report.replayedPrepCases=controls.replayedPrepCases; report.replayedMovementCases=controls.replayedMovementCases;
    report.k15Sensitivity=stage2_bootstrap([point.observedK15 point.expectedK15 point.expectedK15-point.observedK15],controls.indices);
    ...
    report.nullMCSE=zeros(10,2); report.nullMCSEColumns={'Control95','K15_sensitivity'};
    row=0; trials=cell(9600,1); targets=cell(320,1); tr=0;
    for n=1:10
        draws=load(fullfile(old,'cache',sprintf('grid_n%02d.mat',n)),'nullDraws');
        current=load(fullfile(cache,sprintf('alignment_n%02d.mat',n)),'nullValues');
        K=[point.kControl(point.network==n) 15];
        for kk=1:2
            if kk==1, v=current.nullValues; else, v=draws.nullDraws{K(kk)}; end
            assert(numel(v)==10000);
        ...
        s=load(fullfile(cache,sprintf('controls_n%02d.mat',n)),'moves');
        for p=1:4
            m=s.moves{p};
            for q=1:8
