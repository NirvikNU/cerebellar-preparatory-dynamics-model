function audit = stage3_gain_time_check_figure(cfg)
    ...
    for row=1:3
        ...
        for family=1:2
            ...
            finite=expected(isfinite(expected)); limits=[min(finite),max(finite)];
            assert(isequaln(h.CData,expected));
            assert(isequal(h.AlphaData,double(isfinite(expected))));
            errors(row,family)=max(abs(h.CData-expected),[],'all','omitnan');
            assert(errors(row,family)==0 && isequal(ax.CLim,limits));
            assert(strcmp(ax.YDir,'normal'));
            assert(isequal(ax.XLim,[-600 0]));
            assert(isequal(h.XData([1 end]),[-600 0]) && isequal(h.YData([1 end]),[0 6]));
            assert(findobj(ax,'Tag','cue').Value==-500);
            assert(findobj(ax,'Tag','go').Value==0);
            assert(findobj(ax,'Tag','primary_gain').Value==3);
    ...
    assert(numel(findall(f,'Type','colorbar'))==6);
