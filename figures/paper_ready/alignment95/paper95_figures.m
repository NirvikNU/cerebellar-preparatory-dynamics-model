function receipt = paper95_figures(root,allowReviewedReplacement)
    ...
    receipt.figures={}; receipt.sourceData='results/paper_ready/alignment95'; receipt.panelE=false;
    ...
    sgtitle(f,sprintf('Paper a-d | calibrated λ=%g, α=%g, β_{norm}=%g | Control >=95%% PCs | review pending',grid.lambda,point.alpha(1),point.betaNormalized(1)),'FontSize',19);
    receipt=savePair(f,'main_modelling_alignment95',receipt,figRoot,pngRoot);
    ...
    adaptive=stage2_bootstrap(100*[point.observedK15 point.expectedK15],controls.indices);
    grouped(ax,[primary.median;adaptive.median],[primary.se;adaptive.se],[.25 .25 .25;.75 .75 .75], ...
        {'Control 95%','K=15'},{'Observed','Expected'});
    ylabel(ax,'Alignment (%)'); title(ax,'Primary vs K=15 sensitivity');
    sgtitle(f,'Supporting calibration and component-removal time courses | medians +/- network-bootstrap SE','FontSize',19);
    receipt=savePair(f,'support_calibration_alignment95',receipt,figRoot,pngRoot);
    ...
    receipt=savePair(f,'support_noise_controls_alignment95',receipt,figRoot,pngRoot);

    s=load(fullfile(cache,'controls_n01.mat'),'moves'); targetColors=lines(8);
    ...
    for policy=1:4
        ...
        flags=zeros(1,3);
        for n=1:10
            a=load(fullfile(cache,sprintf('controls_n%02d.mat',n)),'moves');
    ...
    receipt=savePair(f,'support_movement_qc_alignment95',receipt,figRoot,pngRoot);
    paper_json(fullfile(root,'artifacts','manifests','paper_ready','alignment95','FIGURES.json'),receipt);
end
